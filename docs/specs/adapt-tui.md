...
- The REPL shows `Adapt: is typing…` while a request is pending. Completed responses are mock-streamed into the terminal by default in word-sized chunks; `/stream` toggles this presentation behavior for the current process. Real MCP progress-event streaming remains a future client enhancement.
